alter table inpatient_bed_profiles add charge_catalog_item_id number(19);
...
create table inpatient_bed_day_facts (
    ...
    bed_location_id number(19) not null,
    ...
    constraint fk_ip_bday_episode foreign key (tenant_id, episode_id) references care_episodes(tenant_id, id),
    constraint fk_ip_bday_encounter foreign key (tenant_id, encounter_id) references encounters(tenant_id, id),
    constraint fk_ip_bday_history foreign key (tenant_id, location_history_id) references encounter_location_histories(tenant_id, id),
    constraint fk_ip_bday_bed foreign key (tenant_id, bed_location_id) references inpatient_bed_profiles(tenant_id, bed_location_id),
...
create index idx_ip_bday_enc_date on inpatient_bed_day_facts (tenant_id, encounter_id, business_date);
