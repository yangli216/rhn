create table settlements (
    ...
    reverses_settlement_id number(19),
    legacy_invoice_id number(19),
    settlement_no varchar2(128 char) not null,
    command_code varchar2(128 char) not null,
    settlement_type varchar2(32 char) not null,
    settlement_scene varchar2(32 char) not null,
    terminal_scene varchar2(32 char) not null,
    status varchar2(32 char) not null,
    gross_amount number(24,6) not null,
    discount_amount number(24,6) not null,
    insurance_amount number(24,6) not null,
    patient_amount number(24,6) not null,
    other_amount number(24,6) not null,
    rounding_amount number(24,6) not null,
    ...
    constraint fk_settlement_account foreign key (tenant_id, patient_account_id) references patient_accounts(tenant_id, id),
    constraint uk_settlement_tenant_id unique (tenant_id, id),
    constraint fk_settlement_reverses foreign key (tenant_id, reverses_settlement_id) references settlements(tenant_id, id),
    constraint fk_settlement_invoice foreign key (tenant_id, legacy_invoice_id) references invoices(tenant_id, id),
...
create table settlement_lines (
    ...
    charge_item_id number(19) not null,
    legacy_invoice_line_id number(19),
    line_no number(10) not null,
    settled_quantity number(28,8) not null,
    gross_amount number(24,6) not null,
    discount_amount number(24,6) not null,
    insurance_amount number(24,6) not null,
    patient_amount number(24,6) not null,
    ...
    constraint fk_stl_line_settlement foreign key (tenant_id, settlement_id) references settlements(tenant_id, id),
    constraint fk_stl_line_charge foreign key (tenant_id, charge_item_id) references charge_items(tenant_id, id),
...
create table settlement_tenders (
    ...
    claim_response_id number(19),
    line_no number(10) not null,
    tender_type varchar2(32 char) not null,
    payer_code varchar2(128 char),
    payer_name_snapshot varchar2(300 char),
    tender_amount number(24,6) not null,
    currency_code varchar2(3 char) not null,
    constraint fk_stl_tender_tenant foreign key (tenant_id) references tenants(id),
    constraint fk_stl_tender_settlement foreign key (tenant_id, settlement_id) references settlements(tenant_id, id),
    constraint fk_stl_tender_payment foreign key (tenant_id, payment_id) references payments(tenant_id, id),
...
create table settlement_events (
    ...
    constraint fk_stl_event_settlement foreign key (tenant_id, settlement_id) references settlements(tenant_id, id),
...
create table settlement_category_summaries (
    ...
    category_name_snapshot varchar2(300 char),
    category_amount number(24,6) not null,
    as_of timestamp with time zone not null,
    constraint fk_stl_cat_tenant foreign key (tenant_id) references tenants(id),
    constraint fk_stl_cat_settlement foreign key (tenant_id, settlement_id) references settlements(tenant_id, id),
...
select i.id, 0, i.tenant_id, i.patient_account_id, i.id, i.invoice_no,
       'LEGACY-INVOICE-' || to_char(i.id),
       case when i.invoice_type = 'CREDIT' then 'REVERSAL' else 'NORMAL' end,
       'OUTPATIENT', 'CASHIER',
       case when i.invoice_type = 'STANDARD' and nvl((select sum(case when p.payment_type = 'PAYMENT' then p.amount else -p.amount end)
              from payments p where p.tenant_id = i.tenant_id and p.invoice_id = i.id), 0) >= i.net_amount then 'SETTLED'
            when nvl((select sum(case when p.payment_type = 'PAYMENT' then p.amount else -p.amount end)
...
insert into settlement_lines (
    ...
    settled_quantity, gross_amount, discount_amount, insurance_amount, patient_amount, other_amount, net_amount
)
...
select i.id, i.tenant_id, i.id, 'CREATE', null, st.status,
       'LEGACY-EVENT-' || to_char(i.id), i.issued_by, i.issued_at
