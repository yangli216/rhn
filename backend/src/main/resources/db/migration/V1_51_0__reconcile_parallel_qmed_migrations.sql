...
create table if not exists RHN_AUD_MED_CAND (
    ID_CAND bigint not null primary key,
    ID_TNT bigint not null,
    ID_USER_ACTOR bigint not null,
    JSON_CONTENT text not null,
    constraint UK_QMED_CAND_TNT unique (ID_TNT, ID_CAND)
);
create table if not exists RHN_AUD_MED_TRIAL (
    ID_TRIAL bigint not null primary key,
    ID_TNT bigint not null,
    ID_CAND bigint not null,
    ID_USER_ACTOR bigint not null,
...
create index if not exists IX_QMED_TRIAL_CAND on RHN_AUD_MED_TRIAL (ID_TNT, ID_CAND, ID_TRIAL);
