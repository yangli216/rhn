create table reconciliation_batches (
    id bigint primary key, revision bigint default 0 not null, tenant_id bigint not null,
    organization_id bigint not null, external_message_id bigint, batch_no varchar(128) not null,
    command_code varchar(128) not null, reconciliation_type varchar(32) not null,
    status varchar(32) not null, source_code varchar(128) not null, payment_method_code varchar(128),
    external_batch_no varchar(128), business_date date not null, local_count int not null,
    external_count int not null, difference_count int not null, local_amount decimal(24,6) not null,
    external_amount decimal(24,6) not null, difference_amount decimal(24,6) not null,
    currency_code varchar(3) not null, created_by bigint not null, created_at timestamp with time zone not null,
    completed_by bigint, completed_at timestamp with time zone,
    ...
    constraint uk_recon_batch_no unique (tenant_id, batch_no),
    constraint uk_recon_batch_command unique (tenant_id, command_code),
...
create index idx_recon_batch_worklist on reconciliation_batches
    (tenant_id, organization_id, status, business_date, source_code);

create table reconciliation_items (
    id bigint primary key, revision bigint default 0 not null, tenant_id bigint not null,
    reconciliation_batch_id bigint not null, payment_id bigint, cashier_close_id bigint, receipt_id bigint,
    external_transaction_no varchar(128), match_type varchar(32) not null, status varchar(32) not null,
    local_amount decimal(24,6) not null, external_amount decimal(24,6) not null,
    difference_amount decimal(24,6) not null, currency_code varchar(3) not null,
    owner_id bigint, resolved_by bigint, resolved_at timestamp with time zone, resolution varchar(2000),
...
create index idx_recon_item_worklist on reconciliation_items
    (tenant_id, reconciliation_batch_id, status, match_type);
...
create table reconciliation_item_events (
    id bigint primary key, tenant_id bigint not null, reconciliation_item_id bigint not null,
    event_type varchar(32) not null, status_from varchar(32), status_to varchar(32) not null,
    command_code varchar(128) not null, actor_id bigint, reason varchar(2000), occurred_at timestamp with time zone not null,
    constraint fk_recon_event_item foreign key (tenant_id, reconciliation_item_id)
        references reconciliation_items(tenant_id, id),
