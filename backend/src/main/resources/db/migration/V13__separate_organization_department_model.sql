create table departments (
    ...
    department_type varchar(128) not null,
    department_property varchar(32),
...
with recursive organization_path
    (tenant_id, unit_id, ancestor_id, parent_id, ancestor_kind, depth) as (
    select tenant_id, id, id, parent_id, organization_kind, 0
    from organizations where organization_kind = 'ORG_UNIT'
    union all
    select p.tenant_id, p.unit_id, o.id, o.parent_id, o.organization_kind, p.depth + 1
    from organization_path p
    join organizations o on o.tenant_id = p.tenant_id and o.id = p.parent_id
    where p.depth < 64
), organization_roots (tenant_id, unit_id, ancestor_id, root_order) as (
    select tenant_id, unit_id, ancestor_id,
           row_number() over (partition by tenant_id, unit_id order by depth) as root_order
...
select u.id, u.tenant_id, r.ancestor_id,
       ...
       coalesce(u.department_type_code, 'CUSTOM_OTHER'),
...
alter table staff_assignments add department_id bigint;
update staff_assignments set department_id = organization_id
where exists (select 1 from departments d
              where d.tenant_id = staff_assignments.tenant_id and d.id = staff_assignments.organization_id);
update staff_assignments set organization_id = (
    select d.organization_id from departments d
    where d.tenant_id = staff_assignments.tenant_id and d.id = staff_assignments.department_id
) where department_id is not null;
alter table staff_assignments alter column department_id set not null;
