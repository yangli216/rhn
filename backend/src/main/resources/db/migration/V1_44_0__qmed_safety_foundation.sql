...
create table RHN_AUD_MED_RULE (
    ID_RULE bigint primary key,
    CD_RULE varchar(100) not null,
    CD_CATEGORY varchar(60) not null,
    NA_RULE varchar(200) not null,
...
create table RHN_AUD_MED_EVAL (
    ID_EVAL bigint primary key,
    ID_TNT bigint not null,
    ID_PRESCRIPTION bigint not null,
    NO_TARGET_REV bigint not null,
    ID_ENC bigint not null,
    ID_PAT bigint not null,
    ID_ORG bigint not null,
    ID_DEPT bigint not null,
    CD_INPUT_HASH varchar(64) not null,
    CD_RULE_SET_VER varchar(100) not null,
    CD_ENGINE_VER varchar(80) not null,
    SD_MODE varchar(24) not null,
    SD_DECISION varchar(24) not null,
...
create table RHN_AUD_MED_FINDING (
    ID_FINDING bigint primary key,
    ID_TNT bigint not null,
    ID_EVAL bigint not null,
    ID_RULE_VER bigint not null,
    SD_SEVERITY varchar(24) not null,
    SD_DECISION varchar(24) not null,
    DES_MESSAGE varchar(1000) not null,
    JSON_REQUEST_IDS text not null,
    JSON_EVIDENCE text not null,
    SD_OVERRIDE_POLICY varchar(24) not null,
    DES_ACTION varchar(1000) not null,
...
create table RHN_AUD_MED_OVERRIDE (
    ID_OVERRIDE bigint primary key,
    ID_TNT bigint not null,
    ID_EVAL bigint not null,
...
insert into RHN_AUD_MED_RULE_VER (ID_RULE_VER, ID_RULE, NO_VERSION, CD_RULE_SET_VER, CD_IMPLEMENTATION, SD_STATUS, SD_SEVERITY, SD_DECISION, SD_OVERRIDE_POLICY, DT_EFFECTIVE_FROM, JSON_EVIDENCE) values (362387869899102, 362387869899101, 1, 'qmed-foundation-shadow-v1', 'java:exact-generic-duplicate:1', 'SHADOW', 'LOW', 'WARN', 'ACKNOWLEDGE', TIMESTAMP '2026-01-01 00:00:00+00:00', '[{"sourceType":"ENGINEERING_BASELINE","sourceTitle":"QMED-1 首条旁路规则规范","sourceVersion":"qmed-duplicate-spec-1","sourceLocator":"docs/architecture/rational-medication/QMED-1接口与验收.md","section":"首条规则语义","excerpt":"同一处方中至少两条 DRAFT/ACTIVE 药品请求具有相同 medicationId 时产生核对提示；不判定治疗不合理。","usageScope":"SHADOW_ONLY; NOT_CLINICAL_EVIDENCE"}]');
