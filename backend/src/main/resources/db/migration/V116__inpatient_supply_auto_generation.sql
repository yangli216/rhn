-- Governed, durable automatic generation of inpatient medication supply batches.
insert into parameter_categories values (
    362387869794030, null, 'INPATIENT', '住院医疗',
    '住院医嘱、护理、病区药品和费用等业务策略', 40, true, 0,
...
insert into parameter_categories values (
    ...
    '住院长期和临时药品医嘱按病区与班次形成供药批次的运行参数', 10, true, 0,
    current_timestamp, 362387869790222, current_timestamp, 362387869790222
);
...
alter table inpatient_med_supply_batches add column generation_trigger varchar(16) default 'MANUAL' not null;
alter table inpatient_med_supply_batches alter column created_by drop not null;
alter table inpatient_med_supply_lines alter column created_by drop not null;
alter table inpatient_med_supply_tasks alter column created_by drop not null;
...
create table inpatient_med_supply_gen_runs (
    ...
    dispense_route_id bigint not null,
    dispense_route_revision bigint not null,
