-- V1_82_0: Add accounting category dimension to charge items and establish accounting category dictionary
...
alter table RHN_BIL_CHARGE_ITEM add column if not exists SD_ACCTG_CAT varchar(64);

comment on column RHN_BIL_CHARGE_ITEM.SD_ACCTG_CAT is '费用核算归并大类(SD_ACCTG_CAT)';

create index if not exists IDX_BIL_CHARGE_ITE_ACCTG_CAT on RHN_BIL_CHARGE_ITEM (ID_TNT, SD_ACCTG_CAT, DT_OCCRD);
...
select 362387869797201, 0, 'PLATFORM', 'PLATFORM', null,
       ...
       timestamp with time zone '2026-09-24 09:00:00+08', null,
       timestamp with time zone '2026-09-24 09:00:00+08', null,
       false, 362387869840006
where not exists (
