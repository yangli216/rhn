...
class StructuredAnalysisPageTest extends RhnIntegrationTestSupport {
    ...
    private long order(long encounter,String kind,String status,String day) {
        ...
        jdbc.update("insert into RHN_EX_CARE_REQ (ID_CARE_REQ,ID_TNT,ID_PAT,ID_ENC,CD_REQ_NO,SD_REQ_KIND,SD_STATUS,CD_INTENT,CD_PRIORITY,ID_CATALOG_ITEM,ID_ORG_PERFORMER,ID_DEPT_PERFORMER,DA_BUSINESS,DT_AUTHORED,ID_USER_AUTHORED,CD_ITEM_SNAP,NA_ITEM_SNAP,CD_UNIT_SNAP,JSON_ITEM_ATTR_SNAP,HASH_ITEM_ATTR,DT_ITEM_ATTR_RESOLVED,JSON_STD_MAP_SNAP) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",id,Long.valueOf(TENANT),PATIENT,encounter,"PLAN"+id,kind,status,"ORDER","ROUTINE",item(),Long.valueOf(ORGANIZATION),Long.valueOf(DEPARTMENT),java.sql.Date.valueOf("2036-01-15"),time,1L,"P01","测试项目","EA","{}","hash",time,"{}");return id;
    }
    private long charge(Long encounter,Long order,String amount,String day,String currency,Long reverses) {
        ...
        jdbc.update("insert into RHN_BIL_CHARGE_ITEM (ID_CHARGE_ITEM,ID_TNT,ID_PAT_ACCT,ID_PAT,ID_ENC,ID_CARE_REQ,ID_CATALOG_ITEM,SD_SRC_TYPE,ID_SRC,CD_REQ,SD_STATUS,QTY_CHARGE,CD_UNIT,PRICE_UNIT,AMT_TOTAL,CD_CURRENCY,CD_ITEM_SNAP,NA_ITEM_SNAP,DT_OCCURRED,ID_USER_ENTERED,ID_CHARGE_ITEM_REVERSES) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",id,Long.valueOf(TENANT),account,PATIENT,encounter,order,item(),"TEST",id,"PLAN"+id,"POSTED",amount.startsWith("-")?-1:1,"EA",100,new java.math.BigDecimal(amount),currency,"P01","测试收费",Timestamp.from(OffsetDateTime.parse(day).toInstant()),1L,reverses);return id;
    }
