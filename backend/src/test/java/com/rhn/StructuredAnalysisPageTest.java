...
class StructuredAnalysisPageTest extends RhnIntegrationTestSupport {
    ...
    private long encounter(long dept,String type) {
        long id=GlobalIds.next();jdbc.update("insert into RHN_VIS_ENC (ID_ENC,ID_TNT,ID_PAT,CD_ENC_NO,ID_ORG,ID_DEPT,SD_STATUS,DT_REGD,SD_ENC_CLASS) values (?,?,?,?,?,?,?,?,?)",id,Long.valueOf(TENANT),PATIENT,"PLAN"+id,Long.valueOf(ORGANIZATION),dept,"IN_PROGRESS",TIME,type);return id;
    }
    ...
    private long order(long encounter,String kind,String status,String day) {
        ...
        jdbc.update("insert into RHN_EX_CARE_REQ (ID_CARE_REQ,ID_TNT,ID_PAT,ID_ENC,CD_REQ_NO,SD_REQ_KIND,SD_STATUS,CD_INTENT,CD_PRI,ID_CATALOG_ITEM,ID_ORG_EXEC,ID_DEPT_EXEC,ID_ORG_REQ,ID_DEPT_REQ,DA_BIZ,DT_AUTHRD,ID_USER_AUTHRD,CD_ITEM_SNAP,NA_ITEM_SNAP,CD_UNIT_SNAP,JSON_ITEM_ATTR_SNAP,HASH_ITEM_ATTR,DT_ITEM_ATTR_RSLVD,JSON_STD_MAP_SNAP) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",id,Long.valueOf(TENANT),PATIENT,encounter,"PLAN"+id,kind,status,"ORDER","ROUTINE",item(),Long.valueOf(ORGANIZATION),Long.valueOf(DEPARTMENT),Long.valueOf(ORGANIZATION),Long.valueOf(DEPARTMENT),java.sql.Date.valueOf("2036-01-15"),time,1L,"P01","测试项目","EA","{}","hash",time,"{}");return id;
    }
