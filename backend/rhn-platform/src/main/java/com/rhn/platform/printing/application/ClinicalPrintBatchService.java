...
public class ClinicalPrintBatchService {
    ...
    public ClinicalPrintBatchService(MedicationPrintSource treatmentService,
    ...
    public BatchView create(CreateBatchCommand command) {
        ...
        List<TaskSnapshot> worklist = treatmentService.worklist("MEDICATION", null, null);
        Map<Long, TaskSnapshot> byId = new HashMap<>();
        worklist.forEach(value -> byId.put(value.id(), value));
        List<TaskSnapshot> selected = command.sourceIds().stream().distinct().map(id -> {
            TaskSnapshot task = byId.get(id);
        ...
        selection.put("sourceIds", selected.stream().map(TaskSnapshot::id).toList());
    ...
    private CandidateView candidate(TaskSnapshot task, ResolvedTemplate resolved,
        ...
        String medicationSummary = task.items().stream().filter(value -> !value.cancelled())
                .map(ItemSnapshot::itemName).reduce((a, b) -> a + "、" + b).orElse("-");
        String routeSummary = task.items().stream().filter(value -> !value.cancelled())
                .map(ItemSnapshot::routeCode).filter(value -> value != null && !value.isBlank())
    ...
    private Map<String, Object> snapshot(TaskSnapshot task, String documentType) {
        List<ItemSnapshot> active = task.items().stream().filter(value -> !value.cancelled()).toList();
        ...
        value.put("medicationName", active.stream().map(ItemSnapshot::itemName)
                .reduce((a, b) -> a + "、" + b).orElse("-"));
        value.put("doseText", active.stream().map(this::doseText).reduce((a, b) -> a + "；" + b).orElse("按医嘱"));
        value.put("routeName", active.stream().map(ItemSnapshot::routeCode)
        ...
        value.put("safetyFlagsText", active.stream().anyMatch(ItemSnapshot::skinTestRequired)
                ? "皮试结果已放行" : "常规核对");
    ...
    private boolean matchesDocumentType(TaskSnapshot task, String documentType) {
        boolean infusion = task.items().stream().filter(value -> !value.cancelled())
                .map(ItemSnapshot::routeCode).anyMatch(this::isInfusionRoute);
