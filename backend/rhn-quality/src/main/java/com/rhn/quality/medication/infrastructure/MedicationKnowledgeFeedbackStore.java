...
public class MedicationKnowledgeFeedbackStore {
    ...
    private Event read(ResultSet rs,int row) throws SQLException {
        ...
        if(!Objects.equals(event.id(),rs.getLong("ID_FEEDBACK"))||event.revision()!=rs.getInt("NO_REVISION")
                ||!Objects.equals(event.basis().runId(),rs.getLong("ID_RULE_RUN"))||!Objects.equals(event.basis().deploymentId(),rs.getLong("ID_DEPLOYMENT"))
                ||!Objects.equals(event.basis().organizationId(),rs.getLong("ID_ORG"))||!Objects.equals(event.basis().departmentId(),rs.getLong("ID_DEPT"))
                ||!Objects.equals(event.operation(),rs.getString("SD_OPERATION"))||!Objects.equals(event.verdict(),rs.getString("SD_VERDICT")))
            throw conflict("QMED_FEEDBACK_INTEGRITY","研判历史索引与快照不一致，请核查审计记录");
        return event;
    }
    public Event latest(Long tenant,Long run) {return jdbc.query("select * from RHN_AUD_KNOW_FEEDBACK where ID_TNT=? and ID_RULE_RUN=? order by NO_REVISION desc fetch first 1 rows only",this::read,tenant,run).stream().findFirst().orElse(null);}
    public PageResult<Event> history(Long tenant,Long run,int page) {
        ...
        var records=jdbc.query("select * from RHN_AUD_KNOW_FEEDBACK where ID_TNT=? and ID_RULE_RUN=? order by NO_REVISION desc offset ? rows fetch next 20 rows only",this::read,tenant,run,(long)page*20);
    ...
    public void append(Long tenant,Event event) {
        ...
        jdbc.update("""
            insert into RHN_AUD_KNOW_FEEDBACK (ID_TNT,ID_FEEDBACK,ID_RULE_RUN,ID_DEPLOYMENT,ID_ORG,ID_DEPT,NO_REVISION,SD_OPERATION,SD_VERDICT,JSON_EVENT,HASH_EVENT)
            values (?,?,?,?,?,?,?,?,?,?,?)
    ...
    private String scope() {return " f.ID_TNT=? and f.ID_ORG=? and f.ID_DEPT=? and f.ID_DEPLOYMENT=? and not exists (select 1 from RHN_AUD_KNOW_FEEDBACK n where n.ID_TNT=f.ID_TNT and n.ID_RULE_RUN=f.ID_RULE_RUN and n.NO_REVISION>f.NO_REVISION)";}
    public Summary summary(Long tenant,Long org,Long dept,Long deployment,long observed) {
        ...
        jdbc.query("select f.SD_VERDICT,count(*) from RHN_AUD_KNOW_FEEDBACK f where "+scope()+" and f.SD_OPERATION='RECORD' group by f.SD_VERDICT",rs->{counts.put(rs.getString(1),rs.getLong(2));},tenant,org,dept,deployment);
