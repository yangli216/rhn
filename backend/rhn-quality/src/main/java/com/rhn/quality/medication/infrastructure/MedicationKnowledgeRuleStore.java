...
public class MedicationKnowledgeRuleStore {
    ...
    public void lockKnowledge(Long tenant,Long knowledge) {
        jdbc.queryForObject("select ID_KNOWLEDGE from RHN_AUD_MED_KNOW_DRAFT where ID_TNT=? and ID_KNOWLEDGE=? and NO_VERSION=1 for update",Long.class,tenant,knowledge);
    }
    public List<KnowledgeRuleCandidate> all(Long tenant) {return jdbc.query("select JSON_CONTENT from RHN_AUD_KNOW_RULE where ID_TNT=? order by ID_KNOWLEDGE,NO_VERSION desc",(r,n)->json.read(r.getString(1),KnowledgeRuleCandidate.class),tenant);}
    ...
    public List<KnowledgeRuleCandidate> versions(Long tenant,Long knowledge) {return jdbc.query("select JSON_CONTENT from RHN_AUD_KNOW_RULE where ID_TNT=? and ID_KNOWLEDGE=? order by NO_VERSION desc",(r,n)->json.read(r.getString(1),KnowledgeRuleCandidate.class),tenant,knowledge);}
    public void append(Long tenant,KnowledgeRuleCandidate value) {
        jdbc.update("insert into RHN_AUD_KNOW_RULE (ID_TNT,ID_KNOW_RULE,ID_KNOWLEDGE,NO_VERSION,NO_KNOWLEDGE_VERSION,CD_COMPILER_VER,JSON_CONTENT) values (?,?,?,?,?,?,?)",
