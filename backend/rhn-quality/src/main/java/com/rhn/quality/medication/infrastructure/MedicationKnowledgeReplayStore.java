...
public class MedicationKnowledgeReplayStore {
    ...
    private Source source(java.sql.ResultSet r) throws java.sql.SQLException {
        ...
        return new Source(r.getLong("ID_EVAL"),r.getLong("ID_PRESCRIPTION"),r.getLong("ID_ENC"),r.getLong("NO_TARGET_REV"),
    ...
    public PageResult<Source> sources(Long tenant,Long org,Long dept,int page) {
        ...
        var rows=jdbc.query("select ID_EVAL,ID_PRESCRIPTION,ID_ENC,NO_TARGET_REV,ID_ORG,ID_DEPT,DT_STARTED,CD_INPUT_HASH,SD_MODE"+where+" order by ID_EVAL desc offset ? rows fetch next 20 rows only",(r,n)->source(r),tenant,org,dept,(long)page*20);
    ...
    public Optional<StoredSource> source(Long tenant,Long org,Long dept,Long id) {
        return jdbc.query("select ID_EVAL,ID_PRESCRIPTION,ID_ENC,NO_TARGET_REV,ID_ORG,ID_DEPT,DT_STARTED,CD_INPUT_HASH,SD_MODE,JSON_INPUT from RHN_AUD_MED_EVAL where ID_TNT=? and ID_ORG=? and ID_DEPT=? and ID_EVAL=?",
    ...
    public void append(Long tenant,Run r) {
        jdbc.update("insert into RHN_AUD_KNOW_REPLAY (ID_TNT,ID_REPLAY,ID_KNOWLEDGE,NO_VERSION,ID_ORG,ID_DEPT,JSON_RUN) values (?,?,?,?,?,?,?)",
    ...
    public Optional<Run> find(Long tenant,Long org,Long dept,Long knowledge,Long id) {
        return jdbc.query("select JSON_RUN from RHN_AUD_KNOW_REPLAY where ID_TNT=? and ID_ORG=? and ID_DEPT=? and ID_KNOWLEDGE=? and ID_REPLAY=?",
    ...
    public PageResult<Summary> history(Long tenant,Long org,Long dept,Long knowledge,int page) {
        String where=" from RHN_AUD_KNOW_REPLAY where ID_TNT=? and ID_ORG=? and ID_DEPT=? and ID_KNOWLEDGE=?";
