...
function visitTypeText(value: ReceptionQueueItem['visitType']) {
  return visitTypeOptions.find((item) => item.value === value)?.label ?? value
}

function queuePresentation(status: ReceptionQueueItem['status']) {
  if (status === 'WAITING') return { label: '候诊中', tone: 'warning' as const }
  if (status === 'IN_SERVICE') return { label: '接诊中', tone: 'info' as const }
  if (status === 'COMPLETED') return { label: '已诊毕', tone: 'success' as const }
...
export function OutpatientRegistrationWorkspace({ api, clinicalContext, onNavigate }: {
  ...
  const registerEncounter = useMutation({
    mutationFn: () => api.encounters.register({
    ...
    onSuccess: async (encounter) => {
      const refreshed = await registrations.refetch()
  ...
  const pageError = linkedResident.error || residents.error || schedules.error || registrations.error || registerEncounter.error
    ...
    <section className="registration-workspace">
      <Panel className="registration-resident-panel">
        ...
        {residents.isFetching ? <LoadingState label="正在检索居民…" /> : !residents.data?.length
          ...
          : <div className="registration-resident-list">{residents.data.map((resident) => <button type="button"
            ...
            onClick={() => { setSelected(resident); setSuccess(null) }}>
      ...
      <Panel className="registration-order-panel">
        ...
        {!selected ? <EmptyState icon="clinical" title="请选择居民" copy="选择左侧居民后即可确认就诊类型和今日排班。" /> : <>
          ...
          <footer className="registration-order-actions"><span>确认后将同步创建挂号单、候诊号和门诊就诊。</span>
            <Button busy={registerEncounter.isPending} busyLabel="正在挂号" disabled={!scheduleId}
              onClick={() => registerEncounter.mutate()}><Icon name="add" />确认挂号</Button></footer>
        </>}
      </Panel>
    </section>
      ...
      {registrations.isPending ? <LoadingState label="正在加载今日挂号…" /> : registrationItems.length === 0
        ...
        : <div className="registration-today-list">
          ...
          {registrationItems.map((item) => {
            const presentation = queuePresentation(item.status)
