...
import type {
  ...
  ServiceSchedule, UpdateScheduleInput,
...
function ProfessionalSchedulingPanel({ api, clinicalContext, today, defaultGenerateDays, practitioners, services, onCreated }: {
  ...
  const createTemplate = useMutation({
    mutationFn: () => api.scheduling.createProfessionalTemplate({
      templateName: templateName.trim(), practitionerId, catalogItemId, dateFrom, dateTo, weekdays,
  ...
  const valid = templateName.trim() && practitionerId && catalogItemId && dateFrom >= today && dateTo >= dateFrom
  ...
  return <>
    ...
    <Panel className="professional-template-panel">
      ...
      {templates.isPending ? <LoadingState label="正在加载专业模板…" /> : !templates.data?.length
        ...
        : <div className="professional-template-list">{templates.data.map((item) => <article key={item.id}>
          <div><strong>{item.templateName}</strong><small>{item.templateCode}</small></div>
          <div><strong>{item.serviceName}</strong><small>{item.practitionerName}</small></div>
