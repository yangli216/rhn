...
it('requires a new preview after direct changes and saves an adjusted function separately',async()=>{
 ...
 await user.click(screen.getByRole('combobox',{name:'统计日期'}));await user.click(screen.getByRole('option',{name:'上个月'}))
...
it('updates the original function as a new version instead of saving a duplicate',async()=>{
 const user=userEvent.setup(),{api,save}=setup()
 api.analytics.savedPages=vi.fn().mockResolvedValue([{id:'v1',functionId:'f1',version:1,spec,savedAt:'2026-09-20'}])
 api.analytics.updatePage=vi.fn().mockImplementation(async(_id,spec)=>({id:'v2',functionId:'f1',version:2,spec,savedAt:'2026-09-20'}))
 render(<DynamicAnalysisLibrary api={api}/> )
 await user.click(await screen.findByRole('button',{name:'编辑原功能'}))
 await user.clear(screen.getByLabelText('功能名称'));await user.type(screen.getByLabelText('功能名称'),'修订统计')
 await user.click(screen.getByRole('button',{name:'保存新版本'}))
 await waitFor(()=>expect(api.analytics.updatePage).toHaveBeenCalledWith('v1',expect.objectContaining({title:'修订统计'})))
 expect(save).not.toHaveBeenCalled()
 expect(within(screen.getByRole('navigation',{name:'已固化统计功能列表'})).getAllByRole('button')).toHaveLength(1)
})
it('renames and archives a function with explicit confirmation and supports restoring it',async()=>{
 const user=userEvent.setup(),{api}=setup()
 const item={id:'v1',functionId:'f1',version:1,spec,savedAt:'2026-09-20'}
 api.analytics.savedPages=vi.fn().mockResolvedValue([item])
 api.analytics.renamePage=vi.fn().mockResolvedValue({...item,id:'v2',version:2,spec:{...spec,title:'新名称'}})
 api.analytics.archivePage=vi.fn().mockImplementation(async(_id,archived)=>({...item,id:archived?'v3':'v4',version:archived?3:4,archived,spec:{...spec,title:'新名称'}}))
 render(<DynamicAnalysisLibrary api={api}/> )
 await user.click(await screen.findByRole('button',{name:'重命名'}))
 await user.clear(screen.getByLabelText('新的功能名称'));await user.type(screen.getByLabelText('新的功能名称'),'新名称')
 await user.click(screen.getByRole('button',{name:'确认重命名'}))
 await waitFor(()=>expect(api.analytics.renamePage).toHaveBeenCalledWith('v1','新名称'))
 await user.click(screen.getByRole('button',{name:'归档功能'}));expect(api.analytics.archivePage).not.toHaveBeenCalled()
 await user.click(screen.getByRole('button',{name:'确认归档'}))
 await screen.findByRole('button',{name:'恢复功能'})
 expect(screen.queryByRole('button',{name:'编辑原功能'})).not.toBeInTheDocument()
 await user.click(screen.getByRole('button',{name:'恢复功能'}));await user.click(screen.getByRole('button',{name:'确认恢复'}))
 await screen.findByRole('button',{name:'编辑原功能'})
 expect(api.analytics.archivePage).toHaveBeenLastCalledWith('v3',false)
})
it('shows older functions with search and pagination and keeps library access in create mode',async()=>{
 const user=userEvent.setup(),{api}=setup()
 api.analytics.savedPages=vi.fn().mockResolvedValue(Array.from({length:61},(_,i)=>({id:`v${i}`,spec:{...spec,title:`功能-${i}`},savedAt:'2026-09-20'})))
 render(<DynamicAnalysisLibrary api={api}/> )
 const list=await screen.findByRole('navigation',{name:'已固化统计功能列表'})
 await waitFor(()=>expect(within(list).getAllByRole('button')).toHaveLength(10))
 await user.type(screen.getByLabelText('查找已固化功能'),'功能-60')
 expect(within(list).getAllByRole('button')).toHaveLength(1)
 await user.click(screen.getByRole('button',{name:'新建统计分析'}))
 expect(screen.getByRole('button',{name:'返回功能库'})).toBeInTheDocument()
})
it('retains original-function edits after a version conflict',async()=>{
 const user=userEvent.setup(),{api}=setup()
 api.analytics.savedPages=vi.fn().mockResolvedValue([{id:'v1',spec,savedAt:'2026-09-20'}])
 api.analytics.updatePage=vi.fn().mockRejectedValue(new Error('功能已被修改，请刷新'))
 render(<DynamicAnalysisLibrary api={api}/> )
 await user.click(await screen.findByRole('button',{name:'编辑原功能'}))
 await user.clear(screen.getByLabelText('功能名称'));await user.type(screen.getByLabelText('功能名称'),'我的修改')
 await user.click(screen.getByRole('button',{name:'保存新版本'}))
 await screen.findByText('功能已被修改，请刷新')
 expect(screen.getByLabelText('功能名称')).toHaveValue('我的修改')
 expect(screen.getByRole('button',{name:'保存新版本'})).toBeEnabled()
})
it('opens an archive-only library and displays the complete version history',async()=>{
 const user=userEvent.setup(),{api,query}=setup()
 api.analytics.savedPages=vi.fn().mockResolvedValue([{id:'v2',functionId:'f1',version:2,archived:true,spec,savedAt:'2026-09-20'}])
 api.analytics.pageHistory=vi.fn().mockResolvedValue([{id:'v2',version:2,archived:true,spec,savedAt:'2026-09-20'},
  {id:'v1',version:1,spec:{...spec,title:'旧名称'},savedAt:'2026-09-19'}])
 render(<DynamicAnalysisLibrary api={api}/> )
