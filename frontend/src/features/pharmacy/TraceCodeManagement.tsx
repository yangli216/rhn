...
export function TraceCodeManagement({ api, siteId, items, bins }: {
  ...
  const metrics = useMemo(() => ({
    total: values.data?.length ?? 0,
    available: values.data?.filter(value => ['AVAILABLE', 'OPENED', 'PARTIALLY_ISSUED'].includes(value.status)).length ?? 0,
    transit: values.data?.filter(value => value.status === 'IN_TRANSIT').length ?? 0,
    exceptions: values.data?.filter(value => ['DAMAGED', 'RECALLED', 'QUARANTINED'].includes(value.status)).length ?? 0,
  }), [values.data])
  return <section className="warehouse-section warehouse-trace">
    ...
    <div className="warehouse-trace-filters">
      <label className="warehouse-search"><span aria-hidden="true">⌕</span><input value={query}
    ...
    <div className="warehouse-metrics warehouse-trace-metrics"><div><span>当前结果</span><strong>{metrics.total}</strong><small>码</small></div>
    ...
    {Boolean(values.error) && <Alert>{errorMessage(values.error)}</Alert>}
    {values.isPending ? <LoadingState label="正在读取追溯码台账…" /> : !values.data?.length
      ...
      : <div className="warehouse-table-wrap"><table className="warehouse-table warehouse-trace-table"><thead><tr>
      ...
      </tr></thead><tbody>{values.data.map(value => <tr key={value.id}>
        ...
        <td><strong>{value.lotNo}</strong><small>余 {value.remainingBaseQuantity} / {value.baseQuantity} {items.find(item => item.id === value.stockItemId)?.baseUnitCode ?? '基本单位'}</small></td>
    ...
    {selected && <Dialog title="追溯码全链路" eyebrow={selected.productName} size="wide"
      ...
      {detail.isPending ? <LoadingState label="正在加载流转轨迹…" /> : <>
        <div className="warehouse-trace-summary"><div><span>当前状态</span><StatusBadge tone={tone(selected.status)}>{statusText[selected.status]}</StatusBadge></div>
          <div><span>药品 / 批号</span><strong>{selected.productName} · {selected.lotNo}</strong></div>
        ...
        <ol className="warehouse-trace-timeline">{detail.data?.events.map(event => <li key={event.id}>
          ...
          <p>{event.documentNo} · {statusText[event.fromStatus ?? ''] ?? event.fromStatus ?? '登记'} → {statusText[event.toStatus] ?? event.toStatus}
            {event.quantityDelta !== 0 ? ` · ${event.quantityDelta > 0 ? '+' : ''}${event.quantityDelta}，余 ${event.balanceAfter}` : ''}</p>
        </li>)}</ol>
      </>}
    </Dialog>}
  </section>
